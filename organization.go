...
func VerifyOrganization(organization interface{}) error {
	response, err := Request(RequestOptions{
		Endpoint: endpoint("organizations/verify"),
		Method:   MethodPost,
		Body: map[string]any{
			"client_id":     config.ID,
...
func ChangeOrganizationStatus(status string, organization interface{}) error {
	response, err := Request(RequestOptions{
		Endpoint: endpoint("organizations/verify"),
		Method:   MethodPost,
		Body: map[string]any{
